...
AnimatedSpriteComponent::AnimatedSpriteComponent(TextureComponent texture, int frame_width,
                                                 int frame_height, float frame_time,
                                                 Vector2 render_dim)
    : texture(texture), frame_width(frame_width), frame_height(frame_height),
      ...
      horizontally_flipped(false), current_animaton_idx(-1) {
...
void AnimatedSpriteComponent::render(SDL_Renderer* renderer, const Vector2& pos,
  ...
  if (horizontally_flipped) {
    SDL_RenderCopyEx(renderer, texture.get_texture(), &src_rect, &dst_rect, 0, nullptr,
                     SDL_FLIP_HORIZONTAL);
  } else {
...
void AnimatedSpriteComponent::add_animation(int animation_idx, const std::string& name,
                                            int start_frame, int end_frame, float frame_time,
                                            bool loop) {
